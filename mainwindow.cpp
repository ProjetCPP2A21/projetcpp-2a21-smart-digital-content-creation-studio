...
#include <QDir>                  // si ce n’est pas déjà inclus
    ...
    , tentative_mdp_oublie(0)
{
    ...
    connect(ui->RechercheRess_2,   &QLineEdit::textChanged, this, &MainWindow::on_RechercheRess_textChanged);
    connect(ui->RessourceTab_2,    &QTableView::clicked, this, &MainWindow::on_RessourceTab_clicked);

    connect(ui->Imagecheck_2, &QCheckBox::checkStateChanged, this, &MainWindow::filterTable);
    connect(ui->Videocheck_2, &QCheckBox::checkStateChanged, this, &MainWindow::filterTable);
    connect(ui->audiocheck_2, &QCheckBox::checkStateChanged, this, &MainWindow::filterTable);
    connect(ui->RessourceTab_2, &QTableView::doubleClicked, this, &MainWindow::on_RessourceTab_doubleClicked);
    // Rendre l'ID en lecture seule (l'utilisateur ne peut pas l'éditer)
    ui->ID_RESSOURCE_2->setReadOnly(true);
    ui->ID_RESSOURCE_2->setStyleSheet("QLineEdit { background-color: #45475a; color: #cdd6f4; }");  // Optionnel : style grisé pour indiquer "non éditable"

    ui->DATECREATION_2->setDate(QDate::currentDate());
    ui->outputajout_2->setReadOnly(true);
    connect(ui->RechercheRess_2, &QLineEdit::textChanged, this, [this](const QString &text) {
    ...
    ui->RechercheRess_2->setPlaceholderText("Rechercher par titre, tags, format...");
...
////////////// ressourcess ///////////////
// -------------------- AJOUT --------------------
QString MainWindow::generateNextId()
{
    QSqlQuery q;
...
void MainWindow::on_ImpBut_clicked()
{
    ...
    ui->outputajout_2->setText(QFileInfo(file).fileName() + " (" + QString::number(currentFileSize/1024.0,'f',1) + " KB)");
    loadFilePreview(file);
}

void MainWindow::loadFilePreview(const QString &path)
{
    ui->labelAffichage_2->clear();
    QPixmap pix(path);
    if (!pix.isNull()) {
        ui->labelAffichage_2->setPixmap(pix.scaled(381,351,Qt::KeepAspectRatio,Qt::SmoothTransformation));
    } else {
        ui->labelAffichage_2->setText("Fichier sélectionné:\n" + QFileInfo(path).fileName());
    }
}

void MainWindow::on_AjoutRess_clicked()
{
    QString id = ui->ID_RESSOURCE_2->text().trimmed();
    QString titre = ui->TITRE_2->text().trimmed();
    QString type = ui->TYPE_2->currentText().trimmed();
    QString tags = ui->TAGS_2->text().trimmed();
    QString format = ui->FORMAT_2->text().trimmed();
    QString dateCreation = ui->DATECREATION_2->date().toString("yyyy-MM-dd");
    QString id_projet = ui->ID_PROJET_2->text().trimmed();
    ...
    if (id.isEmpty())
        id = generateNextId();
    // Après génération...
    ui->ID_RESSOURCE_2->setText(id);  // Affiche l'ID auto-généré avant l'ajout
...
void MainWindow::on_ModifieRess_clicked()
{
    if (ui->ID_RESSOURCE_2->text().isEmpty()) return;
    Ressource r(ui->ID_RESSOURCE_2->text(), ui->TITRE_2->text(), ui->TYPE_2->currentText(),
                ui->TAGS_2->text(), ui->FORMAT_2->text(),
                ui->DATECREATION_2->date().toString("yyyy-MM-dd"),
                ui->ID_PROJET_2->text(), currentFileSize,
...
void MainWindow::on_SupprimerRess_clicked()
{
    QString id = ui->ID_RESSOURCE_2->text();
...
void MainWindow::on_RechercheRess_textChanged(const QString &) { filterTable(); }

void MainWindow::filterTable()
{
    QString search = ui->RechercheRess_2->text();
    bool img = ui->Imagecheck_2->isChecked();
    bool vid = ui->Videocheck_2->isChecked();
    bool aud = ui->audiocheck_2->isChecked();
    ...
    ui->RessourceTab_2->setModel(model);
    ui->RessourceTab_2->resizeColumnsToContents();
}

void MainWindow::on_RessourceTab_clicked(const QModelIndex &index)
{
    int r = index.row();
    ui->ID_RESSOURCE_2->setText(index.sibling(r,0).data().toString());
    ui->TITRE_2->setText(index.sibling(r,1).data().toString());
    ui->TYPE_2->setCurrentText(index.sibling(r,2).data().toString());
    ui->TAGS_2->setText(index.sibling(r,3).data().toString());
    ui->FORMAT_2->setText(index.sibling(r,4).data().toString());
    ui->DATECREATION_2->setDate(QDate::fromString(index.sibling(r,5).data().toString(),"dd/MM/yyyy"));
    ui->ID_PROJET_2->setText(index.sibling(r,6).data().toString());
}

void MainWindow::on_StatistiqueRess_clicked()
{
...
void MainWindow::on_PDFRess_clicked()
{
...
{
    ui->RessourceTab_2->setModel(Ressource::afficher());
    ui->RessourceTab_2->resizeColumnsToContents();
}

void MainWindow::clearForm()
{
    ui->ID_RESSOURCE_2->clear();
    ui->TITRE_2->clear();
    ui->TAGS_2->clear();
    ui->FORMAT_2->clear();
    ui->ID_PROJET_2->clear();
    ui->outputajout_2->clear();
    ui->labelAffichage_2->clear();
    ui->DATECREATION_2->setDate(QDate::currentDate());
...
void MainWindow::highlightSearchResults(const QString &searchText)
{
    if (searchText.trimmed().isEmpty()) {
        // Réinitialise tout
        for (int row = 0; row < ui->RessourceTab_2->model()->rowCount(); ++row) {
            ui->RessourceTab_2->setItemDelegateForRow(row, new QStyledItemDelegate(ui->RessourceTab_2));
        }
        return;
    }
    ...
    QAbstractItemModel *model = ui->RessourceTab_2->model();
    ...
    for (int row = 0; row < model->rowCount(); ++row) {
        ...
        auto *delegate = new QStyledItemDelegate(ui->RessourceTab_2);
        delegate->setProperty("highlight", hasMatch);
        ui->RessourceTab_2->setItemDelegateForRow(row, delegate);
    }
}
